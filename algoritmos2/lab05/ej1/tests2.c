...
void test_tail()
{
    ...
    struct tail_testcase tests[N_TESTCASES_TAIL] = {
        ...
        {{8, 1, -2, 5}, 4, {1, -2, 5}, 3},       // testea: tail([8, 1, -2, 5]) == [1, -2, 5]
        {{8, 1, -2, 5, 7}, 5, {1, -2, 5, 7}, 4}, // testea: tail([8, 1, -2, 5,7]) == [1, -2, 5, 7]
    };
...
void test_addr()
{
    ...
    struct addr_testcase tests[N_TESTCASES_ADDR] = {
        ...
        {{8, 1, -2, 5}, 4, 44, {8, 1, -2, 5, 44}, 5},       // testea: addr([8, 1, -2, 5], 44) == [8, 1, -2, 5, 44]
        {{8, 1, -2, 5, 7}, 5, 44, {8, 1, -2, 5, 7, 44}, 6}, // testea: addr([8, 1, -2, 5,7], 44) == [8, 1, -2, 5, 7,44]
    };
...
void test_take()
{
    ...
    struct take_testcase tests[N_TESTCASES_TAKE] = {
        ...
        {{8, 1, -2}, 3, -1, {}, 0},        // testea: take([8, 1, -2], -1) == []
        {{}, 0, 3, {}, 0},                 // testea: take([], 3) == []
    };
...
void test_drop()
{
    ...
    struct drop_testcase tests[N_TESTCASES_DROP] = {
        ...
        {{8, 1, -2}, 3, -1, {8, 1, -2}, 3}, // testea: drop([8, 1, -2], -1) == [8, 1, -2]
        {{}, 0, 3, {}, 0},                  // testea: drop([], 3) == []
    };
...
void test_concat()
{
    ...
    struct concat_testcase
    {
        int a[MAX_LENGTH];      // elementos de la lista de entrada
        int length;             // largo de la lista de entrada
        int e[MAX_LENGTH];      // lista a agregar
        int e_length;           // largo de la lista a agregar
        int result[MAX_LENGTH]; // elementos esperados de la lista resultado
        int result_length;      // largo esperado de la lista resultado
    };
    ...
    struct concat_testcase tests[N_TESTCASES_CONCAT] = {
        ...
        {{}, 0, {1, 2}, 2, {1, 2}, 2},           // concat([], [1,2]) == [1,2]
        {{1, 2}, 2, {}, 0, {1, 2}, 2},           // concat([1,2], []) == [1,2]
        {{}, 0, {}, 0, {}, 0},                   // concat([], []) == []
    };
    ...
    for (int i = 0; i < N_TESTCASES_CONCAT; i++)
    {
        ...
        // creamos la lista de entrada
        input = array_to_list(tests[i].a, tests[i].length);
        input2 = array_to_list(tests[i].e, tests[i].e_length);
