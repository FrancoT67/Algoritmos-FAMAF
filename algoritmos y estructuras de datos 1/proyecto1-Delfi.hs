...
-- even= comprueva si un numero es par 
...
esPrimo :: Int -> Bool
esPrimo n = existeDivisor n [2..n-1]
...
multiplicaPrimos (x:xs) = productoria' (x:xs) --deberia hacer otra funcion para no usar casos en esta? algo q elija si x es primo o no 
...
--si n pertenece en fib y existe en esFib devuelve True
--esFib :: Int -> Bool 
--esFib n = 
